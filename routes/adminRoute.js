...
router.get('/get-all-users', authMiddleware, async (req, res) => {
    try {
        ...
        res
        .status(200)
        .send({ message: "Users fetched successfully", success: true, data: users });
    } catch (error) {
        console.log("error", error)
        res
        .status(500)
        .send({ message: "Users applying doctor account", success: false , error });
    }
});
...
router.post('/change-doctor-account-status', authMiddleware, async (req, res) => {
    try {
        ...
        res
        .status(200)
        .send({ message: "Doctor status updated successfully", success: true , doctor });
    } catch (error) {
        console.log("error", error)
        res
        .status(500)
